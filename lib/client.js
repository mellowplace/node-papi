...
function Client(opts) {
  ...
  if (opts.promise) {
    this._defer = promise.defer(opts.promise);
  }
}
...
Client.prototype._request = function(opts) {
  ...
  if (this.__request) {
    ...
    var middlewareEnd = request._args.length;

    if (this._defer) {
      var defer = this._defer();
      request._callback = promise.callback(defer);
      returnValue = defer.promise;
    } else if (request._args.length > 1) {
    ...
    request._stack.push.apply(
      request._stack,
      request._args.slice(1, middlewareEnd)
    );
  }
...
Client.prototype.__create = function(request, next) {
  ...
  if (opts.body !== undefined) {
    ...
    var isStream = request.body instanceof stream.Readable;
  ...
  if (opts.pipe) {
    ...
    if (!(request.pipe instanceof stream.Writable)) {
...
Client.prototype.__execute = function(request, next) {
  ...
  req.on('error', function(err) {
    ...
    if (done) {
      if (abort) request.ctx.removeListener('done', abort);
